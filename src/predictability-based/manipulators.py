...
class Manipulator:
    ...
    def move_in_cartesian(self, position, orientation, speed=1.0, sleep=False, ignore_force=True):
        current_position, current_orientation = self.get_tip_pose()
        length = np.linalg.norm(current_position-np.array(position))
        t = (length * 2)/speed

        N = int(t * 240)

        position_traj = np.linspace(current_position, position, N+1)[1:]
        # TODO: we need to do quaternion interpolation here
    ...
    def get_contacted_object(self):
        # hand_contacts = self._p.getContactPoints(bodyA=self.id, linkIndexA=27)
        # hand_contacts = [c for c in hand_contacts if c[2] != 7]
        finger1_contacts = list(self._p.getContactPoints(bodyA=self.id, linkIndexA=28))
        finger2_contacts = list(self._p.getContactPoints(bodyA=self.id, linkIndexA=29))
        contacts = finger1_contacts + finger2_contacts
        contacts = [c[2] for c in contacts if c[2] != 7]
        if len(contacts) == 0:
            return -1
        contact_obj = np.unique(contacts)[0]
        return contact_obj
